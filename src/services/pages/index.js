...
class Pages {
  ...
  async unsubscribeWebhook(
    { access_token, facebookPageId, id },
    facebookUserId
  ) {
    const facebook = new Facebook(access_token, facebookUserId);
    await facebook.unsubscribeFromPage({
      facebookPageId: facebookPageId,
      access_token
    });
    let updated = await this.db.update(
      {
        subscribed_to_webhook: false
      },
      {
        where: {
          id
  ...
  async createOrUpdateFacebookPages(facebookPages, userId, theme) {
    const promises = facebookPages.map(fbPage =>
      this.updateOrCreatePage(fbPage, userId, theme)
    );
