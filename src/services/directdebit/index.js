...
class DirectDebit {
  ...
  async createRedirectFlow(user) {
    ...
    await this.saveRedirectId(
      user,
      response.redirect_flows.id,
      response.redirect_flows.redirect_url
    );
    return {
  ...
  async cancelSubscription(UserId) {
    ...
    const response = await gcRequest(
      `mandates/${record.mandateId}/actions/cancel`
    );
    record.active = false;
  ...
  async subscribeCustomerToPlan(user, mandateId) {
    ...
    const plan = plans.find(p => p.id == "ekko-monthly");
    ...
    if (useCoupon && useCoupon.discount < 100) {
      let discountedAmount = parseInt(
        plan.cost - plan.cost * (useCoupon.discount / 100),
        10
      );
      let discountedSub = {
      ...
      const newDate = moment(discountedResponse.subscriptions.end_date)
        .add(1, "month")
        .format("YYYY-MM-DD");
      fullPriceSub.start_date = newDate;
    } else if (useCoupon && useCoupon.discount == 100) {
      const newDate = moment()
        .add(useCoupon.months, "month")
        .format("YYYY-MM-DD");
      fullPriceSub.start_date = newDate;
    }
    ...
    logger.info(
      "New Direct Debit subscription",
      fullPriceResponse.subscriptions
    );
    await directDebitSubscriptions.addSubscription({
  ...
  async completeRedirectFlow(user) {
    ...
    const response = await gcRequest(
      `redirect_flows/${redirectId}/actions/complete`,
      {
        body: JSON.stringify({
          data: {
            session_token: user.id.toString()
