...
const getConvertedDomainCost = async domainCost =>
  await cc.convert(domainCost, "USD", "GBP");
...
class Stripe {
  ...
  async validateCoupon(coupon) {
    let validatedCoupon;
    try {
      logger.info("Checking Stripe coupon", coupon);
      validatedCoupon = await stripe.coupons.retrieve(coupon);
    } catch (e) {
      validatedCoupon = null;
    }
    return validatedCoupon;
  }
  async createCustomer({ id, email, coupon }, token, plan) {
    let customer;
    let validCoupon;
    if (coupon) {
      let checkedCoupon;
      try {
        checkedCoupon = await this.validateCoupon(coupon);
      } catch (e) {
        checkedCoupon = null;
      }
      if (checkedCoupon) {
    ...
    const subscription = await this.subscribeCustomerToPlan(customer.id, plan);
    let { sources } = customer;
    let planId = subscription ? subscription.plan.id : null,
      subscriptionId = subscription.id,
      activeUntil = moment
        .utc(subscription.current_period_end, "X")
        .format("YYYY-MM-DD");
  ...
  async subscribeCustomerToPlan(customerId, plan) {
    ...
    //   activeUntil = moment
    //     .utc(subscribedCustomer.current_period_end, "X")
    //     .format("YYYY-MM-DD");
    // const updatedCustomer = await this.db.update(
    //   {
    //     customerId,
  ...
  async addChargeToCustomer(customerId, domainCost) {
    ...
    logger.info(
      "Created Stripe charge on customer",
      charge,
      customerId,
      amount
    );
