...
export default class Funk extends React.Component {
  ...
  render() {
    ...
    const showRestaurantSpecialties =
      restaurant_specialties &&
      Object.keys(restaurant_specialties).some(
        key => restaurant_specialties[key]
      );

    const getRestaurantSpecialties = () => {
      return Object.keys(restaurant_specialties)
        .filter(key => restaurant_specialties[key])
        .join(", ");
    };

    const showRestaurantServices =
      restaurant_services &&
      Object.keys(restaurant_services).some(key => restaurant_services[key]);
    ...
    const getRestaurantServices = () => {
      return Object.keys(restaurant_services)
        .filter(key => restaurant_services[key])
        .map(service => {
          return (
    ...
    const showInfoPanel =
      showRestaurantServices || showRestaurantSpecialties || hours;
    ...
    const outputHours = hours => {
      ...
      return Object.keys(days).map((dayKey, index) => {
        return [
          <div className="info-panel__times__day">{days[dayKey]}:</div>,
    ...
    showGallery &&
      (childElements = photos.slice(0, 9).map(function(photo) {
    ...
    showPosts &&
      (childPostElements = posts.slice(0, 6).map(function(post) {
        const postId = post.id.match(/_/) ? post.id.split("_").pop() : null;
        const postURL = postId
          ? `https://facebook.com/${id}/posts/${postId}`
        ...
        return (
          <li className="masonry-wrap__entry">
            <div className="masonry-wrap__entry__blog">
              ...
              <p className="masonry-wrap__blog-text">
        ...
        <div className="intro">
          ...
          <header className="site-header">
            <div className="container">
              ...
              {tel && <tel className="site-header__phone">{tel}</tel>}
            </div>
          </header>
          <div className="container">
            <div className="grid">
              <div className="grid__item one-whole lap--four-fifths desk--three-quarters push--desk--one-twelfth">
                ...
                {about && <p className="emphasised blue-light">{about}</p>}
              </div>
            </div>
          </div>
        </div>
                ...
                <div className="grid__item palm--one-whole lap--three-quarters desk--three-quarters">
                  {events.map(event => {
                    return (
                      <article>
                        <h3 className="no-mb">
                          <a href={`https://facebook.com/${event.id}`}>
                            {event.name}
                          </a>
                        </h3>
                        <p className="meta">
                          {moment(event.start_time).format(
                            "dddd, Do MMMM YYYY [at] H:mm"
                          )}
                        </p>
                ...
                {(location.street || location.city || location.zip) &&
                  this.state.showMap && (
                    <div className="site-footer__map">
                      <iframe
                        src={`https://www.google.com/maps/embed/v1/place?key=${API_KEY}&q=${vals(
                          (({ street, city, country }) => ({
                            street,
                            city,
                            country
                ...
                {(screennames || []).length && (
                  <p className="site-footer__social">
                    {(screennames || [])
                      .filter(
                        s =>
                          ["Instagram", "Twitter"].indexOf(s.service_name) > -1
                      )
                      .concat([
                        {
                      ...
                      .map(s => {
                        return (
                          <a
                            href={`http://${s.service_name.toLowerCase()}.com/${
                              s.value
                            }`}
                            className={`site-footer__social__link site-footer__social__link--${s.service_name.toLowerCase()}`}>
                            {s.service_name}
                          </a>
                        );
                      })}
