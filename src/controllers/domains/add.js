...
const cc = require("currency-converter")({
  CLIENTKEY: process.env.OPEN_EXCHANGE_RATES
});
...
const add = {
  async addDomain({
    ...
    let parsedDomain = parseDomain(domainName);
    if (!parsedDomain) {
      throw new ApiError(400, "That domain isn't valid I'm afraid");
    }

    const domainTLD = parsedDomain.tld;
    ...
    if (
      existingPageWithDomain &&
      existingPageWithDomain.domain !== domainName
    ) {
      throw new ApiError(400, "That page already has a domain name assigned");
    }

    // Pre-save domain
    const preDomainSave = await domains.addDomain(
      domainName,
      PageId,
      shouldRegister
    );
    ...
    if (shouldRegister) {
      ...
      const registeredDomain = await domains.registerDomain(
        domainName,
        newContact
      );

      // Add domain to Page
      const addedDomain = await domains.addDomainWithPage(
        domainName,
        PageId,
        registeredDomain.period,
        registeredDomain.id,
        newContact
      );
      ...
      if (domainCost > 14) {
        ...
        if (user.StripeCustomerId) {
          chargedForDomain = await stripeService.addChargeToCustomer(
            user.StripeCustomerId.customerId,
            domainCost
          );
        } else if (user.DirectDebitCustomerId) {
          chargedForDomain = await directDebit.addPaymentToCustomer(
            user.id,
            domainCost
          );
        }
      ...
      const domainRequest = await domainRequests.add(preDomainSave.id, user);
    }
    ...
    if (
      addedDomainToHeroku &&
      addedDomainToHeroku.hostname &&
      addedDomainToHeroku.hostname == domainName
    ) {
      await domains.addedToHeroku(preDomainSave.id);
    }

    const userPages = await pages.getByUserId(user.id);
    const userDomains = await domains.getByPageIds(
      userPages.map(page => page.get("id"))
    );
