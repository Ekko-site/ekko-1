...
const slack = new Slack("https://hooks.slack.com/services/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")

const send = ({ text }) => slack.send({ text: `${process.env.NODE_ENV !== 'production' ? '[development]' : ''} ${text}` })
...
export default {
    ...
    newDomainAdded: ({ domainName, userId, registered }) => {
        send({
            text: `The user ${userId} added the domain ${domainName} to their Ekko account. It ${registered ? 'was' : 'wasn\'t'} registered through DNSimple.`
        })
    },
