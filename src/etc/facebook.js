...
class Facebook {
    ...
    async fetchPages(opts = {
        ...
        const promises = res.data.map(page => this.fetchPage(page.id, page.access_token))
    ...
    async fetchPhotos(data, pageId, preview) {
        ...
        const queried = data.sort((a, b) => {
            return new Date(b.created_time) - new Date(a.created_time)
        }).slice(0, 20).map(photo => {
            return {
                ...photo,
        ...
        }).filter(photo => photo.images.length).filter(photo => ['Cover Photos', 'Profile Pictures'].indexOf(photo.album.name) == -1)
    ...
    async fetchPosts(data, pageId, preview) {
        ...
        const queried = data.filter(post => post.message).sort((a, b) => {
            return new Date(b.created_time) - new Date(a.created_time)
        }).slice(0, 10)
    ...
    async fetchEvents(data, preview) {
        ...
        const filtered = data.filter(event => +new Date(event.start_time) >
        +new Date())
    ...
    async fetchPage(id, token = this.token, subscribe = true, opts = {
        preview: false
    }) {
        FB.setAccessToken(token)
        const preview = opts.preview
        const methods = (['fetchPage', 'fetchPhotos', 'fetchPosts', 'fetchPicture', 'fetchEvents', 'fetchCallToActions', 'fetchScreenames'].concat(subscribe ? ['fetchSubscribeToPage']: null)).filter(Boolean)

        let res,
            pageFields = ['id', 'name', 'about', 'cover', 'category', 'description', 'access_token', 'hours', 'link', 'location', 'phone', 'emails', 'restaurant_services', 'restaurant_specialties'],
            batch = ([
                { method: 'get', relative_url: `${id}?fields=${pageFields.join(',')}` },
                { method: 'get', relative_url: `${id}/photos/uploaded?fields=${['images', 'album', 'created_time', 'name', 'width'].join(',')}` },
                { method: 'get', relative_url: `${id}/posts?fields=${['full_picture', 'object_id', 'message', 'created_time'].join(',')}` },
                { method: 'get', relative_url: `${id}/picture?type=large&redirect=false` },
                { method: 'get', relative_url: `${id}/events?fields=${['cover', 'name', 'start_time', 'end_time', 'description', 'place'].join(',')}` },
                { method: 'get', relative_url: `${id}/call_to_actions?fields=${['web_url', 'type', 'status', 'web_destination_type'].join(',')}` },
                { method: 'get', relative_url: `${id}/screennames` }
            ].concat(subscribe ? [{ method: 'post', relative_url: `${id}/subscribed_apps`, body: `access_token=${token}` }] : null)).filter(Boolean)
        try {
        ...
        if(res.some(r => r.code !== 200)){
            logger.error('error fetching facebook page', id, this.facebook_id, res.filter(r => r.code !== 200).map(r => ({ body: JSON.parse(r.body), headers: r.headers })))
            throw new ApiError(502, 'Bad Facebook request')
        }
        const bodies = res.map(r => JSON.parse(r.body))
        const data = await reduce(bodies, async (accum, current, index) => {
            const methodName = methods[index]
            const method = this[methodName]
            if(methodName == 'fetchPage'){
                current.cover = current.cover && !preview ? await this.storeCover(current.cover, id) : current.cover
                return {
                    ...accum,
                    ...current
                }
