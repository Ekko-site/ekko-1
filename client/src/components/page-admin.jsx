...
const findPage = pageState =>
  pageState.pages.find(p => p.id == pageState.current);
  ...
  getPageOnline(page, togglePageOnline, pageSync) {
    ...
    return (
      <p className="no-mb faded mini">
        <span className="inline-icon sync" /> Last synced{" "}
            ...
            <div className="grid__item one-whole lap--one-half desk--one-half">
              ...
              {!page.online && (
                <p className="no-mb faded mini red">
                  <span className="inline-icon sync" /> Website offline
                </p>
              )}
              {page.online &&
