        ...
        <form
          ...
          onSubmit={event => {
            ...
            if (existingInput.value) {
              onDomainSelection(existingInput.value, false, undefined);
            }
          }}>
          <div className="half-mb">
            <label className="mini faded" for="existing-domain-input">
              Add a domain to {page.data.name}
            </label>
            <input
              ref={c => (existingInput = c)}
              type="text"
              className="mini half-mb"
              id="existing-domain-input"
              placeholder="abc.com"
            />
            {/* <label className="mini faded" for="registrar-input">
              Do you know who you purchased your domain name from? (optional)
            </label>
            <input
              ref={c => (registrarInput = c)}
              type="text"
              className="mini"
              id="registrar-input"
              placeholder="e.g. 123 Reg, GoDaddy, etc..."
            /> */}
          </div>
          <button className="butt butt--existing-domain">Submit</button>
        </form>
      </div>
    </div>
  );
