...
const homepageInit = () => {
    ...
    setTimeout(function() {
        ...
        $(".fb-circle-img").addClass("flipInX").delay(1000).queue(function(next){
            next();
            $(".loader").addClass("spinning").delay(1000).queue(function(next){
                next();
                $(".home-intro__headline__fb-page").removeClass("active");
                $(".loader").addClass("hide");
                $(".triangle").addClass("fadeInUpLeft").delay(600).queue(function(next){
                    next();
                    $(".home-intro__headline__create-website").addClass("active");
                    $(".big-circle").addClass("bounceIn").delay(800).queue(function(next){
                        next();
                        $(".shape-wrapper").addClass("moveback").delay(400).queue(function(next){
                            next();
                            $(".shape-wrapper").addClass("bounceOut").delay(750).queue(function(next){
                                next();
                                $(".home-intro__headline").addClass("pushup");
                                $(".home-intro__headline__create-website").removeClass("active");
                                $(".site-logo-link__logomark").addClass("fadeInLeft").delay(500).queue(function(next){
    ...
    setInterval(function(){
        $('.just-works__bg-area > :first-child').fadeOut()
        .next('div').fadeIn()
        .end().appendTo('.just-works__bg-area');},
    3000);
