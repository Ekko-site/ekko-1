...
class Dashboard extends React.Component {
    ...
    componentDidMount() {
        if (this.props.authState.current_user && this.props.authState.current_user.facebookUserId && !this.props.pageState.pages.length) {
    ...
    componentWillReceiveProps(nextProps) {
        if (!this.props.authState.current_user && nextProps.authState.current_user && nextProps.authState.current_user.facebookUserId && !this.props.pageState.pages.length) {
            this.props.pageActions.pagesFetch()
            this.props.domainActions.userPagesFetch()
        }
    }
    ...
    render() {
        const { pages, current, page_fetched, page_fetching } = this.props.pageState
        const { current_user, existing_facebook_user, fetchingFacebookPage, facebookPages } = this.props.authState
        ...
        return <div>
            ...
            />
            {(page && theme && page_fetched && !page_fetching && current_user && current_user.facebookUserId) && (
                <div>
                    {(page) && (
                        <Preview
                            url={pageUrl}
            ...
            {(!page && !page_fetching && !pages.length && !page_fetched && !current_user) && (
                <ApiError />
            )}
        </div>
    }
...
function mapStateToProps(state) {
    return {authState: state.authState, pageState: state.pageState, themesState: state.themesState, domainState: state.domainState}
}
