...
export function directDebitInit() {
  return {
    type: types.DIRECT_DEBIT_INIT
  };
}

export function completingDirectDebitInit() {
  return {
    type: types.COMPLETING_DIRECT_DEBIT_INIT
  };
}
  ...
  return (dispatch, getState) => {
    ...
    return fetch("api/users/upgrade", {
      ...
      if (json.user) {
        ...
        toastr.success(
          "Congratulations",
          "You're now a fully paid Ekko user. Let's get a domain name connected to your website."
        );
      }
    });
  };
...
export function completeDirectDebit() {
  return dispatch => {
    dispatch(completingDirectDebitInit());
    return fetch("api/directdebit/complete").then(json => {
      if (json.error) {
        return dispatch(upgradeErrors(json.error));
      }
      if (json.user) {
...
export function setupDirectDebit() {
  return (dispatch, getState) => {
    dispatch(directDebitInit());
    return fetch("api/directdebit/initiate").then(json => {
      if (json.error) {
        return dispatch(upgradeErrors(json.error));
      }
...
export function changePassword(new_password, reset_hash) {
  return dispatch => {
    ...
    return fetch("api/users/change-password", {
      ...
      toastr.success(
        "Password changed",
        "You can now login with your new Ekko password."
      );
      dispatch(go("LOGIN"));
    });
  };
...
export function deleteAccount() {
  return dispatch => {
    ...
    return fetch("api/users/cancel").then(json => {
      toastr.info(
        "Ok",
        "Your account has been deleted. It's been emotional. Goodbye."
      );
    ...
    return fetch("api/users/login", {
      ...
      .then((json = {}) => {
        if (json.error) {
          ...
          cancelLogin();
          return setTimeout(
            () => dispatch(formActions.setErrors("login", false)),
            3000
          );
        }
        if (json.user) {
...
export function signUp({ email, password, firstName, lastName, coupon }) {
  return (dispatch, getState) => {
    const { authState } = getState();

    const cancelSignUp = () =>
      dispatch(formActions.change("signUp.signingUp", false));

    dispatch(formActions.change("signUp.signingUp", true));
    if (!email) {
      cancelSignUp();
      return dispatch(
        formActions.batch("signUp", [
    ...
    return fetch("api/users/sign-up", {
      ...
      .then((json = {}) => {
        if (json.error) {
          ...
          cancelSignUp();
          return setTimeout(
            () => dispatch(formActions.setErrors("signUp", false)),
            3000
          );
        }
        if (json.user) {
...
export function facebookFetchPage(
  params,
  opts = {
    fromHomepage: false
  }
...
export function facebookConnect({ facebook_id, facebook_access_token }) {
  return (dispatch, getState) => {
    dispatch(signUpInit());
    const { authState } = getState();
    let body = {
